...
IMPORTANT: Ansible requires that Python2 be installed on all the target machines or many of the modules will not work. On the `Alpha` and `Bravo` install the following packages:

. `software-properties-common`
. `python`

`Charlie` and `Delta`, being Amazon Linux, already have Python installed.
